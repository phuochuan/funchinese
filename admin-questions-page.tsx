...
interface Question {
  ...
  category: string | null; audioUrl: string | null; createdAt: string;
}
...
function QuestionModal({ initial, onClose, onSaved }: {
  ...
  const submit = async () => {
    ...
    const res    = await fetch(url, {
      method,
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ...form, options, audioUrl: audioUrl || null }),
    });
    setSaving(false);
                ...
                <td className="px-5 py-4 max-w-sm">
                  <div className="flex items-start gap-3">
                    <div className="flex-1 min-w-0">
                      ...
                      {q.meaningVi && (
                        <p className="text-xs text-on-surface-variant italic truncate">{q.meaningVi}</p>
                      )}
                    </div>
                    {q.audioUrl && (
