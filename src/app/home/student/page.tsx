...
function WelcomeBanner({ data }: { data: DashboardData | null }) {
  ...
  return (
    <div className="bg-gradient-to-br from-primary/10 to-primary/5 rounded-2xl p-5 md:p-6 flex flex-col lg:flex-row gap-5 overflow-hidden relative border border-primary/15">
      {/* <div className="absolute right-32 top-0 text-[8rem] md:text-[10rem] chinese-text font-bold text-primary/[0.06] leading-none select-none pointer-events-none">学</div> */}
      ...
      <div className="flex-1">
        {data ? (
          <>
            ...
            <p className="text-xs md:text-sm text-on-surface-variant leading-relaxed mb-4 max-w-sm">
              {pct >= 100
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5">
            <span className="text-xs font-semibold text-on-surface-variant">XP hôm nay</span>
            <span className="text-xs font-bold text-primary">{xpEarned} / {xpGoal} XP</span>
          </div>
      ...
      {/* <div className="bg-white rounded-xl p-4 md:p-5 lg:w-60 flex-shrink-0 border border-outline-variant/20 shadow-sm">
        <p className="text-xs font-bold text-on-surface-variant uppercase tracking-wider mb-2">Tiếp tục học tập</p>
        {lesson ? (
          <>
            <h3 className="font-bold text-on-surface mb-1 text-sm line-clamp-2">{lesson.title}</h3>
            {lesson.titleChinese && (
              <p className="chinese-text text-xl text-primary mb-0.5">{lesson.titleChinese}</p>
            )}
            {lesson.pinyin && (
              <p className="text-xs text-on-surface-variant mb-3">{lesson.pinyin}</p>
            )}
            <Link href={`/home/student/lessons/${lesson.id}`}
              className="flex items-center justify-center gap-2 w-full bg-primary text-on-primary font-bold text-sm py-2.5 rounded-xl hover:opacity-90 transition-all">
              Học ngay
          ...
          <div className="text-center py-3">
            <p className="text-sm text-on-surface-variant mb-3">Chưa có bài đang học</p>
            <Link href="/home/student/courses"
              className="inline-flex items-center gap-1 bg-primary text-on-primary font-bold text-xs py-2 px-4 rounded-xl hover:opacity-90 transition-all">
              Bắt đầu học
